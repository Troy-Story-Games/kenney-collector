...
func trigger_action():
	...
	if len(bodies) > 0:
		for body in bodies:
			if body is RigidBody3D or body is OptionCrate:
				held_object = body
				break
	...
	held_object.connect("tree_exiting", Callable(self, "_on_held_object_tree_exiting"))
...
func trigger_released_action():
	...
	held_object.apply_impulse(controller.controller_velocity, Vector3(0, 0, 0))
	held_object = null
